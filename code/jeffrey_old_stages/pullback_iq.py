...
def gt_seeds(cfg, blk, d, K, proc, S, means_t, R99, device):
    ...
    L = gt["gt"].long().reshape(-1).to(device)
    ...
    if agree < float(blk.get("min_agree", 0.9)):
        print("[gt] seeds not reproduced; using the model's own labels instead")
        return None
...
def pullback_one(cfg, d, K, device):
    ...
    if got is not None:
        ...
        hall_rate = float((Lall < 0).float().mean())
        Z = Zall[Lall < 0]                                   # GT-hallucinated seeds only
    else:
        Z = proc.seeds(int(pc.n_seeds), d, int(pc.seed) + 17)
        n_total, hall_rate, gt_src = None, None, "model labels"
    X0 = S.G(Z)
    D = torch.cdist(X0, means_t)
    dmin = D.min(1).values
    hall = dmin > R99
    if n_total is None:
        n_total, hall_rate = Z.shape[0], float(hall.float().mean())
    ...
    info = {"d": d, "K": K, "T": T, "R99": R99, "field": str(pc.field),
            "iq_energy": str(pc.iq_energy), "n_seeds": int(n_total), "ground_truth": gt_src,
            "hall_rate": hall_rate, "deep_rate": float(deep.sum()) / max(1, n_total),
            "n_study": Sn}
...
SUMMARY_KEYS = ["d", "K", "ground_truth", "n_seeds", "n_study", "hall_rate", "deep_rate", "lam", "iq_rescue", "ours_rescue",
...
def _anim(D, J, path, fps=25, hold=20, intro=20, max_frames=120, dpi=110):
    ...
    K, R99, MU, t = J["K"], float(D["R99"]), D["means"], D["t"]
    ok = (D["lab_iq"] >= 0) & (D["lab_ours"] >= 0)
    if ok.any():                                             # a seed both methods fixed
        j = int(np.flatnonzero(ok)[0])
    elif (D["lab_ours"] >= 0).any():                         # else one ours fixed (IQ may not have)
        j = int(np.flatnonzero(D["lab_ours"] >= 0)[0])
    else:
        return None
    Tr = {k: D[k][:, j] for k in ("T_og", "T_iq", "T_ours")}
    n = len(t)
...
def sweep_one(cfg, d, K, device):
    ...
    n_h = int(hall.sum())
    N = int(Z.shape[0])
    row = {"d": d, "K": K, "T": int(ck["T"]), "n_eval": N, "R99": R99, "ground_truth": gt_src,
           "base_hall_rate": n_h / N, "n_hall": n_h}
    if n_h == 0:
        print(f"[pullback_sweep] d={d:>2} K={K:>2}: no hallucinations in {N} seeds")
        return row
    zh = Z[hall]
    tgt = torch.cdist(S.G(zh), means_t).argmin(1)            # nearest mode of each endpoint
    mu_t = means_t[tgt]

    gc = int(pc.get("grad_chunk", 64))                       # backprop through all T steps
    nrm = torch.cat([pulled_normal(S, c, S.T - 1, m)[0]
                     for c, m in zip(zh.split(gc), mu_t.split(gc))])
...
SWEEP_KEYS = ["d", "K", "T", "ground_truth", "n_eval", "n_hall", "base_hall_rate", "hall_rate_after",
