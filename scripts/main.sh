  ...
  while [ $done_n -lt $total ]; do
    while [ ${#FREE[@]} -gt 0 ] && [ $i -lt $total ]; do
      ...
      ( for ((try=0; try<=RETRIES; try++)); do
          ...
          CUDA_VISIBLE_DEVICES="$gpu" python code/main.py $args device="$DEVICE" > "$LOGDIR/$log" 2>&1 \
            && { echo "  [ok]   $log"; exit 0; }
...
for P in $PROCESSES; do
  for D in $(echo $DIMS | tr ' ' '\n' | sort $SORTFLAG); do
    for K in $(echo $KS | tr ' ' '\n' | sort $SORTFLAG); do
      JOBS+=("${STAMP}_${P}_train_d${D}_K${K}.log|process=$P process.T_train=$TTRAIN sweep.d=[$D] sweep.K=[$K] seed=$SEED n_seeds=$NSEEDS data.radius=$RADBASE stages=[train] train.force_retrain=$FORCE train.graph_streams=$STREAMS run_id=$STAMP $EXTRA")
    done
  done
done
...
for P in $PROCESSES; do
  for T in $TS; do
    JOBS+=("${STAMP}_${P}_T${T}.log|process=$P process.T_train=$TTRAIN process.T_true=$T sweep.d=$DLIST sweep.K=$KLIST sweep.anchors=$ANCHORS seed=$SEED n_seeds=$NSEEDS data.radius=$RADBASE stages=[evaluate,visualize] train.force_retrain=false run_id=$STAMP $EXTRA")
  done
done
